...
applyjob(index, event){
    axios.get('http://localhost:5000/applicant/getbyid/'+applid)
    .then((res) => {
        if(res.data.applications>=10)
        {
        ...
        if(sop==null)return;
        var words=sop.split(" ");
        if(words.length>250){alert("SOP SHOULD BE LESSER THAN 250 WORDS. TRY AGAIN");}
        else{
        this.setState({sop: sop});
        const UserData = {
                job_id: this.state.jobs[index]._id,
                applicant_id: applid,
                sop: sop
            }
